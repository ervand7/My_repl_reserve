...
# # 2) read line - построчное чтение
# # 3) read lines (.readlines()) - чтение всех строк в список

# # 1) read - чтение всего файла целиком. Подробнее.
# with open('test_2.txt') as f:
#   data = f.read() # метод .read() берет все данные из файла и читает их в переменную f. Этот метод плохо работает, если файл весит очень много. И поэтому есть другие метода (см. стр. 64)
#   print(type(data))
#   print(data)
# data += '\nЕще одна строка'
# print(data)

# # 2) read line - построчное чтение. Подробнее.
# with open('test_2.txt') as f:
...
# # 3) read lines (.readlines()) - чтение всех строк в список. Подробнее.
# with open('test_2.txt') as f:
#   lines = f.readlines()
#   print(len(lines))
#   print(lines[1])
...
# Режимы чтения/записи. Это не альтернативы, а дополнения к 'r', 'w' и 'a'
# 'rb' (read binary) - двоичный режим (работаем с байтами)
# 't' (text) - текстовый режим (работаем со строками, по-умолчанию)
...
# Теперь 2 раза подряд запишем данные в файл и видим, что каждый раз, когда мы открываем файл в режиме 'w' (write), мы каждый раз его открываем в режиме перезаписи. И все прежнее содержимое стерается
# with open('test_3.txt', 'w') as f:
...
# 1. Абсолютный (от корневого каталога) - используем реже относительного
# Работаете с конкретной директорией?
# Используйте абсолютные пути
# Этот путь нужен для запуска файлов, которые всегда в неизменном месте дежат

# 2. Относительный (от текущего каталога)
# Нужен временный файл или конкретный путь не задан?
# Используйте относительные пути
# Чаще всег придется использовать относительный путь
