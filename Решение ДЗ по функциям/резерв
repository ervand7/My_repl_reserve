...
def add_shelf():
  a = input('Введите номер полки, которую вы хотите создать, чтобы она была затем автоматически добавлена в общий список полок: ')
...
def move():
  a = input('Введите номер документа, для того, чтобы можно было его в дальнейшем переместить с текущей полки на другую, которую вы выберите: ')
...
def add():
  ...
  new_for_shelf = input_doc_number # Эта переменная создана для того, чтобы в список с полками добавлять номер нового документа
  new_for_documents = {"type": input_type, "number": input_doc_number, "name": imput_owner} # Ну к эта переменная создана, соответственно6 чтобы в список с документами добавить новый словарь

  if input_shelf_number in directories.keys(): # Этот иф прописывем лишь для того, чтобы в крайнем случае вывести дефолтное значение
# А тут пришлось дико хитрить, чтобы избежать None с условием отсутствия принтов. Для того, чтобы ретурн у нас возвращал только s_1 пришлось второй список (каталог общий) прописывать в коде перед первым, чтобы потом в первом (который у нас список полок) писать в f-строке ссылку на второй список просто указанием переменной {s}
...
# Как видно, здесь не пришлось мутить дополнительный иф наверху для ретурна дефолтного значения. Это из-за того, что у нас 2 подряд идентичных цикла

#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

# А здесь мы распишем функцию чисто для кнопок. 
# Внимание!!!!!!!!!!!! Здесь не нужен return с его дефолтными значениями. Эта функция ничего возвращать не будет. Мы импользуем простой else для указания ошибки пользовательского ввода
def main():
